...
Pharm3DFPCalculator::Pharm3DFPCalculator(): DescriptorCalculator("PHARM_3D", BITSET, true)
{}

Pharm3DFPCalculator::~Pharm3DFPCalculator()
{}

void Pharm3DFPCalculator::addOptions(CmdLineLib::CmdLineBase& cl_base)
{
    cl_base.addOption("pharm-3d-size",
                      "Size of the generated fingerprint (default: 8191).",
                      cl_base.value<std::size_t>(&size));
    cl_base.addOption("pharm-3d-min-tuple-size",
                      "Minimum feature tuple size (default: 1).",
                      cl_base.value<std::size_t>(&minTupleSize));
    cl_base.addOption("pharm-3d-max-tuple-size",
    ...
    cl_base.addOption("pharm-3d-bin-size",
                      ...
                      cl_base.value<double>()->notifier(
                          [&](double bin_size) {
                              if (bin_size <= 0.0)
                                  cl_base.throwValidationError("pharm-3d-bin-size");
                              
                              this->binSize = bin_size;
                          }));
}

void Pharm3DFPCalculator::getOptionSummary(std::string& summary) const
{
    summary.append("Pharm. 3D FP Bit Count;");
    summary.append(std::to_string(size));
    summary.push_back(';');
    summary.append("Pharm. 3D FP Min. Ftr. Tuple Size;");
    summary.append(std::to_string(minTupleSize));
    summary.push_back(';');
    summary.append("Pharm. 3D FP Max. Ftr. Tuple Size;");
    summary.append(std::to_string(maxTupleSize));
    summary.push_back(';');
    summary.append("Pharm. 3D FP Bin Size;");
    summary.append((boost::format("%.3f") % binSize).str());
    summary.push_back(';');
}

DescriptorCalculator* Pharm3DFPCalculator::clone() const
{
    auto copy = new Pharm3DFPCalculator();
...
void Pharm3DFPCalculator::prepare(CDPL::Chem::Molecule& mol) const
{
    CDPL::Pharm::prepareForPharmacophoreGeneration(mol);
}

void Pharm3DFPCalculator::calculate(const CDPL::Chem::MolecularGraph& molgraph, CDPL::Util::BitSet& fp)
{
    if (!fpGenImpl) {
        fpGenImpl.reset(new CDPL::Descr::NPoint3DPharmacophoreFingerprintGenerator());
