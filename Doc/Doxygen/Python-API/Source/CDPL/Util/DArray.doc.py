...
class DArray(Boost.Python.instance):
    ...
    # 
    # Different Python \c %DArray instances may reference the same underlying C++ class instance. The commonly used Python expression
    # <tt>a is not b</tt> thus cannot tell reliably whether the two \c %DArray instances \e a and \e b reference different C++ objects. 
    ...
    ##
    # \brief Replaces the current state of \a self with a copy of the state of the \c %DArray instance \a array.
