...
# 
# The set of atom and bond properties used to break ties during numbering can be configured via setAtomPropertyFlags() and setBondPropertyFlags(). A custom function used for retrieving atom hydrogen counts in the preprocessing step can be installed via setHydrogenCountFunction().
# 
# \see [\ref MCKAY]
# 
class CanonicalNumberingCalculator(Boost.Python.instance):
    ...
    # 
    # Different Python \c %CanonicalNumberingCalculator instances may reference the same underlying C++ class instance. The commonly used Python expression
    # <tt>a is not b</tt> thus cannot tell reliably whether the two \c %CanonicalNumberingCalculator instances \e a and \e b reference different C++ objects. 
