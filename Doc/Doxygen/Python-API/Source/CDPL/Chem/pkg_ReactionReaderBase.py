...
##
# \brief An interface for reading data objects of type Chem.Reaction from an arbitrary data source.
# 
# <tt>ReactionReaderBase</tt> is the common interface of classes which read objects of type Chem.Reaction from some data source (e.g. a file) that provides the data encoded in a particular storage format.
# 
# From the <tt>ReactionReaderBase</tt> interface point of view, the data source is organized as an array of logical data records. Each record is addressed by a zero-based sequential record index and contains the data for exactly <em>one</em> data object. The total number of available data records (which is equal to the maximum record index plus <em>1</em>) can be queried by the method getNumRecords().
# 
# Similar to low-level file I/O, data records may either be read in a sequential or in a random access manner. For sequential access the method read(rxn: Reaction) is provided which will read the data record at the 'current' record index (similar to a file reference). The current record index is accessible by the method getRecordIndex() and can be modified using the method setRecordIndex() (similar to a seek operation).
# 
# For reading data records in a random access manner, the method read(std::size_t idx, rxn: Reaction) is available. In contrast to the former read() method, this method expects the index of the record to read as the first argument.
# 
...
class ReactionReaderBase(Base.DataIOBase):
    ...
    ##
    # \brief Reads the data record at the current record index and stores the read data in <em>rxn</em>.
    # 
    # If the read operation was successful, the record index is incremented by <em>1</em>.
    # 
    # \param rxn The Reaction object storing the read data.
    # \param overwrite Specifies whether any existing data in <em>rxn</em> shall be replaced by the newly read data or if the read data should be appended (if supported by the reader and data type).
    # 
    # \return \a self 
    # 
    ...
    ##
    # \brief Reads the data record at index <em>idx</em> and stores the read data in <em>rxn</em>.
    # 
