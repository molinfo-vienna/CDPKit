...
# 
# Bonds mappings are stored as pairs of references to the mapped Chem.Bond objects. Mappings do not have to be unique and multiple mappings of a given bond to other bonds are possible. If a mapping entry for a particular bond does not exist, the methods BondMapping.getValue() and BondMapping.__getitem__() return <tt>None</tt> to indicate that the lookup of the mapped bond has failed.
# 
