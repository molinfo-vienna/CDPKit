...
##
# \brief An interface for reading data objects of type Chem.Molecule from an arbitrary data source.
# 
# <tt>MoleculeReaderBase</tt> is the common interface of classes which read objects of type Chem.Molecule from some data source (e.g. a file) that provides the data encoded in a particular storage format.
# 
# From the <tt>MoleculeReaderBase</tt> interface point of view, the data source is organized as an array of logical data records. Each record is addressed by a zero-based sequential record index and contains the data for exactly <em>one</em> data object. The total number of available data records (which is equal to the maximum record index plus <em>1</em>) can be queried by the method getNumRecords().
# 
...
class MoleculeReaderBase(CDPL.Base.DataIOBase):
    ...
    # \param self The \e %MoleculeReaderBase instance to initialize.
    # 
    def __init__() -> None: pass

    ##
    # \brief Reads the data record at the current record index and stores the read data in <em>mol</em>.
    # 
    # If the read operation was successful, the record index is incremented by <em>1</em>.
    # 
    # \param mol The Molecule object storing the read data.
    # \param overwrite Specifies whether any existing data in <em>mol</em> shall be replaced by the newly read data or if the read data should be appended (if supported by the reader and data type).
    # 
    # \return \a self 
    # 
    ...
    ##
    # \brief Reads the data record at index <em>idx</em> and stores the read data in <em>mol</em>.
    # 
