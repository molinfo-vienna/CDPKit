...
#  <tr><td>Chem.AtomProperty.MATCH_CONSTRAINTS</td><td>Specifies atom matching constraints</td><td>Only written if the constraints are user-defined and can be expressed by corresponding <em>JME</em> constraints</td></tr>
#  <tr><td>Chem.AtomProperty.COORDINATES_2D</td><td>Specifies the 2D position of the atom</td><td>-</td></tr>
#  <tr><td>Chem.AtomProperty.TYPE</td><td>Specifies the type of the atom (see Chem.AtomType)</td><td>Only written if the atom is a query atom, a corresponding matching constraint has been specified and the type is supported by the <em>JME</em> format.</td></tr>
...
# </table>
# 
# <tt>JMEMolecularGraphWriter</tt> can be configured with the following control-parameters:
# 
# <table>
...
#  <tr><td>Chem.ControlParameter.JME_SEPARATE_COMPONENTS</td><td><tt>True</tt></td><td>Specifies whether the components of a moleculargraph shall be written separately or combined as a single <em>JME</em> component</td></tr>
# </table>
# 
class JMEMolecularGraphWriter(MolecularGraphWriterBase):

    ##
    # \brief Constructs a <tt>JMEMolecularGraphWriter</tt> instance that will write data of molecular graphs to the output stream <em>os</em>.
    # 
