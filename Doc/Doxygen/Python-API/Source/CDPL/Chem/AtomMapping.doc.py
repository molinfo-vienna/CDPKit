...
# 
# Atoms mappings are stored as pairs of references to the mapped Chem.Atom objects. Mappings do not have to be unique and multiple mappings of a given atom to other atoms are possible. If a mapping entry for a particular atom does not exist, the methods AtomMapping.getValue() and AtomMapping.__getitem__() return <tt>None</tt> to indicate that the lookup of the mapped atom has failed.
# 
class AtomMapping(Boost.Python.instance):
    ...
    # 
    # Different Python \c %AtomMapping instances may reference the same underlying C++ class instance. The commonly used Python expression
    # <tt>a is not b</tt> thus cannot tell reliably whether the two \c %AtomMapping instances \e a and \e b reference different C++ objects. 
