...
##
# \brief An interface for reading data objects of type Grid.DRegularGridSet from an arbitrary data source.
# 
# <tt>DRegularGridSetReaderBase</tt> is the common interface of classes which read objects of type Grid.DRegularGridSet from some data source (e.g. a file) that provides the data encoded in a particular storage format.
# 
# From the <tt>DRegularGridSetReaderBase</tt> interface point of view, the data source is organized as an array of logical data records. Each record is addressed by a zero-based sequential record index and contains the data for exactly <em>one</em> data object. The total number of available data records (which is equal to the maximum record index plus <em>1</em>) can be queried by the method getNumRecords().
# 
...
# 
# For reading data records in a random access manner, the method read(idx: int, grid_set: DRegularGridSet) is available. In contrast to the former read() method, this method expects the index of the record to read as the first argument.
# 
# If a read() (or skip()) operation was successful, the current record index is updated to point to the record immediately following the just read (or skipped) data record. If an operation fails, the reader instance is set into an error state that can be queried by the special methods __bool__() and __nonzero__(). Additionally, a <tt>DRegularGridSetReaderBase</tt> implementation may decide to throw an exception of type Base.IOError to report the error condition.
# 
class DRegularGridSetReaderBase(Base.DataIOBase):
    ...
    ##
    # \brief Reads the data record at the current record index and stores the read data in <em>grid_set</em>.
    # 
    # If the read operation was successful, the record index is incremented by <em>1</em>.
    # 
    # \param grid_set The DRegularGridSet object storing the read data.
    # \param overwrite Specifies whether any existing data in <em>grid_set</em> shall be replaced by the newly read data or if the read data should be appended (if supported by the reader and data type).
    # 
    # \return \a self 
    # 
    ...
    ##
    # \brief Reads the data record at index <em>idx</em> and stores the read data in <em>grid_set</em>.
    # 
