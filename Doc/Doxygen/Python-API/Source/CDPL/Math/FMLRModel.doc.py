...
# 
# \f[ y_i = \beta_1 x_{i1} + \beta_2 x_{i2} + \ldots + \beta_M x_{iM} + \epsilon_i \f]
# 
...
# 
# \f[ \chi^2 = \sum_{i=1}^{N} (y_i - f(\vec{X}_i, \vec{\beta}))^2 \f]
# 
...
class FMLRModel(Boost.Python.instance):
    ...
    # 
    # Different Python \c %FMLRModel instances may reference the same underlying C++ class instance. The commonly used Python expression
    # <tt>a is not b</tt> thus cannot tell reliably whether the two \c %FMLRModel instances \e a and \e b reference different C++ objects. 
    ...
    ##
    # \brief Replaces the current state of \a self with a copy of the state of the \c %FMLRModel instance \a model.
    # \param model The \c %FMLRModel instance to copy.
    # \return \a self
    # 
    ...
    # 
    def getCoefficients() -> FVector: pass
    ...
    # 
    # \f[ \chi^2 = \sum_{i=1}^{N} (y_i - \sum_{j=1}^{M}(x_{ij} \beta_j))^2 \f]
    # 
    ...
    # 
    # \f[ Q = gammaq(\frac{N - 2}{2}, \frac{\chi^2}{2}) \f]
    # 
    ...
    # 
    # \f[ r = \frac{\sum_{i=1}^{N} (\hat{y}_i - \bar{\hat{y}})(y_i - \bar{y})} {\sqrt{\sum_{i=1}^{N} (\hat{y}_i - \bar{\hat{y}})^2 \sum_{i=1}^{N} (y_i - \bar{y})^2 }} \f]
    # 
    # where
    # 
    ...
    # 
    # \f[ s_r = \sqrt{\frac{\sum_{i=1}^{N} (y_i - \sum_{j=1}^{M} (x_{ij} \beta_j))^2} {N - M}} \f]
    # 
