...
class Vector3FArrayAlignmentCalculator(Boost.Python.instance):
    ...
    # 
    # Different Python \c %Vector3FArrayAlignmentCalculator instances may reference the same underlying C++ class instance. The commonly used Python expression
    # <tt>a is not b</tt> thus cannot tell reliably whether the two \c %Vector3FArrayAlignmentCalculator instances \e a and \e b reference different C++ objects. 
