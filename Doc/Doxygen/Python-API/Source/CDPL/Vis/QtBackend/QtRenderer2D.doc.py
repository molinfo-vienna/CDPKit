...
# 
# <tt>QtRenderer2D</tt> is implemented by forwarding the drawing operations to a provided <tt>QPainter</tt> instance which has been created for the desired output device (screen, printer, off-screen image, ...).
# 
# \see [\ref QTDOC] for more information about <tt>QPainter</tt> and the <em>Qt Toolkit</em>.
# 
class QtRenderer2D(Vis.Renderer2D):

    ##
    # \brief Constructs a QtRenderer2D instance which uses the <tt>QPainter</tt> instance <em>painter</em> for its drawing operations.
    # 
