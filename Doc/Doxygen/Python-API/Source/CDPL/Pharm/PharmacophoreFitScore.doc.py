...
class PharmacophoreFitScore(Boost.Python.instance):
    ...
    # 
    # \param query_mode Forwarded to the embedded Pharm.SpatialFeatureMapping. If <tt>True</tt>, the reference feature container is interpreted as a query pharmacophore so that position matching uses only the reference feature's tolerance as the maximum allowed distance; if <tt>False</tt>, the maximum of the two feature tolerances is used.
