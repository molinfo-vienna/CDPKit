...
class PatternBasedFeatureGenerator(FeatureGenerator):
    ...
    ##
    # \brief Role-flags assigned (via the atom-mapping-ID property) to atoms of feature substructure patterns added through addIncludePattern(). The flags are bit-OR-combinable so that a single pattern atom can act in several roles simultaneously.
    # 
