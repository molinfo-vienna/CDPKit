...
class CircularFingerprintGenerator(Boost.Python.instance):
    ...
    # 
    # \param include If <tt>True</tt>, atom chirality is considered during fingerprint generation.
    # 
    ...
    # 
    # \return <tt>True</tt> if atom chirality is considered, and <tt>False</tt> otherwise.
    # 
    ...
    ##
    # \brief Returns the identifier of the feature at index <em>ftr_idx</em>.
    # 
    ...
    ##
    # \brief Returns the atom-bit mask describing the substructure covered by the feature at index <em>ftr_idx</em>.
    # 
    # In the returned bitset, the bit at position <em>i</em> is set if the atom with index <em>i</em> is part of the feature substructure.
    # 
    # \param ftr_idx The zero-based feature index.
    # 
    # \return A reference to the atom-bit mask.
    # 
    def getFeatureSubstructure(ftr_idx: int) -> Util.BitSet: pass

    ##
    # \brief Extracts the substructure covered by the feature at index <em>ftr_idx</em> into <em>frag</em>.
    # 
    # \param ftr_idx The zero-based feature index.
    # \param frag The output fragment.
    # \param clear If <tt>True</tt>, <em>frag</em> is cleared before atoms and bonds are added.
    # 
    def getFeatureSubstructure(ftr_idx: int, frag: Chem.Fragment, clear: bool = True) -> None: pass

    ##
    # \brief Extracts the substructures of every feature that, when folded into a bitset of size <em>bs_size</em>, maps to the bit index <em>bit_idx</em>.
    # 
    # \param bit_idx The target bit index.
    # \param bs_size The bitset size used for the folding.
    # \param frags The output fragment list.
    # \param clear If <tt>True</tt>, <em>frags</em> is cleared before any fragments are appended.
    # 
