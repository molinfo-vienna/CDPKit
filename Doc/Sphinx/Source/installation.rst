...
.. rubric:: CDPKit build requirements
...
.. note::
   ...
   *Windows 8/10/11* all packages can be obtained from the `MSYS2 <https://www.msys2.org/>`_ software distribution.
   `Sphinx <https://www.sphinx-doc.org/en/master/>`_ and all required extensions can be installed via :program:`pip`.

.. rubric:: Makefile generation and compilation

Makefiles are generated by executing :program:`cmake`:
...
If :program:`cmake` is executed as shown the default build type will be *Release*. Configuring
...
If the makefiles have been generated without errors, the compilation process is started by:

.. code-block:: bash

   $ make

.. rubric:: Installation of the generated binaries
...
will install CDPKit in a platform specific default directory (*Linux:* ``/opt``, *macOS:*, *Windows:*).
