...
from CDPL.Base import ValueKey
...
class TestCase(unittest.TestCase):
	...
	def checkProperties(self, key):
        ...
        self.assert_(key.__str__() == key.getName())
        self.assert_(key.numericID == key.getID())

	def runTest(self):
		"""Testing ValueKey"""

		checkProperties(ValueKey.NONE);
				
		self.assert_(ValueKey.NONE.getName() == 'NONE');

		# ---------

		key1 = ValueKey.create('key1');

		checkProperties(key1);

		self.assert_(key1.getName() == 'key1');

		key2 = ValueKey.create('key2');

		checkProperties(key2);
		...
		key3 = ValueKey.create('keyX');

		checkProperties(key3);
		...
		key4 = ValueKey.create('');

		checkProperties(key4);
