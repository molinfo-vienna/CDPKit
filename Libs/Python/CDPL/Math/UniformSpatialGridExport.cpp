...
namespace
{
    ...
    struct UniformSpatialGridExport
    {

		UniformSpatialGridExport(const char* name) {
			...
			python::class_<GridType, typename GridType::SharedPointer>(name, python::no_init)
				.def(python::init<const GridType&>((python::arg("self"), python::arg("usg"))))
				...
				.def("getCoordinatesTransform", &GridType::getCoordinatesTransform, python::arg("self"), 
					 python::return_internal_reference<>())
				.def("getCoordinates", &GridType::template getCoordinates<python::object>, 
...
void CDPLPythonMath::exportUniformSpatialGridTypes()
{
    using namespace CDPL;

    UniformSpatialGridExport<Math::UniformSpatialGrid<float> >("FUniformSpatialGrid");
    UniformSpatialGridExport<Math::UniformSpatialGrid<double> >("DUniformSpatialGrid");
}
