...
const double GRAILS::InteractionScoreGridCalculator::DEF_DISTANCE_CUTOFF = 10.0;


GRAILS::InteractionScoreGridCalculator::InteractionScoreGridCalculator(): 
	scoreCombinationFunc(MaxScoreFunctor()), distCutoff(DEF_DISTANCE_CUTOFF), normScores(true)
{}

GRAILS::InteractionScoreGridCalculator::InteractionScoreGridCalculator(const ScoringFunction& func): 
	scoringFunc(func), scoreCombinationFunc(MaxScoreFunctor()), distCutoff(DEF_DISTANCE_CUTOFF), normScores(true)
{}

GRAILS::InteractionScoreGridCalculator::InteractionScoreGridCalculator(const ScoringFunction& scoring_func, const ScoreCombinationFunction& comb_func): 
	scoringFunc(scoring_func), scoreCombinationFunc(comb_func), distCutoff(DEF_DISTANCE_CUTOFF), normScores(true)
{}

GRAILS::InteractionScoreGridCalculator::InteractionScoreGridCalculator(const InteractionScoreGridCalculator& calc):
	scoringFunc(calc.scoringFunc), scoreCombinationFunc(calc.scoreCombinationFunc), distCutoff(calc.distCutoff) {}

GRAILS::InteractionScoreGridCalculator::~InteractionScoreGridCalculator() {}

void GRAILS::InteractionScoreGridCalculator::setDistanceCutoff(double dist)
{
	distCutoff = dist;
}

double GRAILS::InteractionScoreGridCalculator::getDistanceCutoff() const
{
	return distCutoff;
}

void GRAILS::InteractionScoreGridCalculator::normalizeScores(bool normalize)
{
	normScores = normalize;
}

bool GRAILS::InteractionScoreGridCalculator::scoresNormalized() const
{
	return normScores;
}

void GRAILS::InteractionScoreGridCalculator::setScoringFunction(const ScoringFunction& func)
{
    scoringFunc = func;
}

const GRAILS::InteractionScoreGridCalculator::ScoringFunction& GRAILS::InteractionScoreGridCalculator::getScoringFunction() const
{
    return scoringFunc;
}

void GRAILS::InteractionScoreGridCalculator::setScoreCombinationFunction(const ScoreCombinationFunction& func)
{
    scoreCombinationFunc = func;
}

const GRAILS::InteractionScoreGridCalculator::ScoreCombinationFunction& GRAILS::InteractionScoreGridCalculator::getScoreCombinationFunction() const
{
    return scoreCombinationFunc;
}

GRAILS::InteractionScoreGridCalculator& GRAILS::InteractionScoreGridCalculator::operator=(const InteractionScoreGridCalculator& calc) 
{
	if (this == &calc)
		return *this;
...
void GRAILS::InteractionScoreGridCalculator::calculate(const Pharm::FeatureContainer& features, Grid::DSpatialGrid& grid)
{
	calculate(features, grid, FeaturePredicate());
}

void GRAILS::InteractionScoreGridCalculator::calculate(const Pharm::FeatureContainer& features, Grid::DSpatialGrid& grid, const FeaturePredicate& tgt_ftr_pred)
{
