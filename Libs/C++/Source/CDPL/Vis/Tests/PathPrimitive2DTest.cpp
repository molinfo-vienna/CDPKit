...
namespace
{
    ...
    void checkClone(const CDPL::Vis::PathPrimitive2D& prim)
    {
        using namespace CDPL;
        using namespace Vis;

        GraphicsPrimitive2D::SharedPointer gp_clone_ptr = prim.clone();
        const PathPrimitive2D* prim_clone_ptr = static_cast<const PathPrimitive2D*>(gp_clone_ptr.get());
...
BOOST_AUTO_TEST_CASE(PathPrimitive2DTest)
{
    using namespace CDPL;
    using namespace Vis;
...
# ifdef HAVE_CAIRO_PNG_SUPPORT
    ...
    BOOST_CHECK(cairo_surface_write_to_png(surf_ptr.get(), "PathPrimitive2DTest.png") == CAIRO_STATUS_SUCCESS);
