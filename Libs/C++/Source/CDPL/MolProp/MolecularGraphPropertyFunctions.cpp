...
#define MAKE_MOLGRAPH_PROPERTY_FUNCTIONS(PROP_NAME, TYPE, FUNC_SUFFIX)        \
    TYPE MolProp::get##FUNC_SUFFIX(const Chem::MolecularGraph& molgraph)      \
    {                                                                         \
        return molgraph.getProperty<TYPE>(MolecularGraphProperty::PROP_NAME); \
    }                                                                         \
                                                                              \
    void MolProp::set##FUNC_SUFFIX(Chem::MolecularGraph& molgraph, TYPE arg)  \
    {                                                                         \
        molgraph.setProperty(MolecularGraphProperty::PROP_NAME, arg);         \
    }                                                                         \
                                                                              \
    bool MolProp::has##FUNC_SUFFIX(const Chem::MolecularGraph& molgraph)      \
    {                                                                         \
        return molgraph.isPropertySet(MolecularGraphProperty::PROP_NAME);     \
    }                                                                         \
                                                                              \
    void MolProp::clear##FUNC_SUFFIX(Chem::MolecularGraph& molgraph)          \
    {                                                                         \
        molgraph.removeProperty(MolecularGraphProperty::PROP_NAME);           \
    }


MAKE_MOLGRAPH_PROPERTY_FUNCTIONS(FUNCTIONAL_GROUPS, const Chem::FragmentList::SharedPointer&, FunctionalGroups)
