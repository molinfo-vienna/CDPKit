...
BOOST_AUTO_TEST_CASE(MMFF94BondStretchingInteractionAnalyzerTest)
{
	...
	while (mol_reader.read(mol)) {
		...
		for (std::size_t i = 0; i < iactions.size(); i++) {
			...
			for (std::size_t j = 0; j < found_iactions.getSize(); j++) {
				...
				if ((iaction.getAtom1Index() == iactions[i].atom1Idx && iaction.getAtom2Index() == iactions[i].atom2Idx) ||
					...
					BOOST_CHECK_MESSAGE(std::abs(iaction.getReferenceLength() - iactions[i].refLength) < 0.0005, 
										"Reference bond length mismatch for bond stretching interaction between atom #" << iactions[i].atom1Idx << " (" << 
										getMOL2Name(mol.getAtom(iactions[i].atom1Idx)) << ") and  atom #" << iactions[i].atom2Idx << " (" << 
					...
					BOOST_CHECK_MESSAGE(std::abs(iaction.getForceConstant() - iactions[i].forceConst) < 0.0005, 
										"!! Force constant mismatch for bond stretching interaction between atom #" << iactions[i].atom1Idx << " (" << 
										getMOL2Name(mol.getAtom(iactions[i].atom1Idx)) << ") and  atom #" << iactions[i].atom2Idx << " (" << 
										getMOL2Name(mol.getAtom(iactions[i].atom2Idx)) << ") of molecule #" << mol_idx << " (" << mol_name <<
			...
			BOOST_CHECK_MESSAGE(iaction_found, "!! Bond stretching interaction between atom #" << iactions[i].atom1Idx << " (" << getMOL2Name(mol.getAtom(iactions[i].atom1Idx)) << 
								") and  atom #" << iactions[i].atom2Idx << " (" << getMOL2Name(mol.getAtom(iactions[i].atom2Idx)) << ") of molecule #" << mol_idx << " (" << mol_name <<
