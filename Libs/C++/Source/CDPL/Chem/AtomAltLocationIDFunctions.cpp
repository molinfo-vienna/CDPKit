...
char Chem::getAltLocationID(const Atom& atom)
{
    return atom.getProperty<char>(AtomProperty::ALT_LOCATION_ID);
}

void Chem::setAltLocationID(Atom& atom, char id)
{
    atom.setProperty(AtomProperty::ALT_LOCATION_ID, id);
}

void Chem::clearAltLocationID(Atom& atom)
{
    atom.removeProperty(AtomProperty::ALT_LOCATION_ID);
}

bool Chem::hasAltLocationID(const Atom& atom)
{
