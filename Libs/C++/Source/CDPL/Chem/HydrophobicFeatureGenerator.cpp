...
void Chem::HydrophobicFeatureGenerator::setFeatureType(unsigned int type)
{
	featureType = type;
}

unsigned int Chem::HydrophobicFeatureGenerator::getFeatureType() const
{
	return featureType;
}

void Chem::HydrophobicFeatureGenerator::setFeatureTolerance(double tol)
{
	featureTol = tol;
}

double Chem::HydrophobicFeatureGenerator::getFeatureTolerance() const
{
	return featureTol;
}

void Chem::HydrophobicFeatureGenerator::setFeatureGeometry(unsigned int geom)
{
	featureGeom = geom;
}

unsigned int Chem::HydrophobicFeatureGenerator::getFeatureGeometry() const
{
	return featureGeom;
}

void Chem::HydrophobicFeatureGenerator::setRingHydrophobicityThreshold(double thresh)
{
	hydThreshRing = thresh;
}

double Chem::HydrophobicFeatureGenerator::getRingHydrophobicityThreshold() const
{
	return hydThreshRing;
}

void Chem::HydrophobicFeatureGenerator::setChainHydrophobicityThreshold(double thresh)
{
	hydThreshChain = thresh;
}

double Chem::HydrophobicFeatureGenerator::getChainHydrophobicityThreshold() const
{
	return hydThreshChain;
}

void Chem::HydrophobicFeatureGenerator::setGroupHydrophobicityThreshold(double thresh)
{
	hydThreshGroup = thresh;
}

double Chem::HydrophobicFeatureGenerator::getGroupHydrophobicityThreshold() const
{
	return hydThreshGroup;
}

void Chem::HydrophobicFeatureGenerator::addNonPatternFeatures(const MolecularGraph& molgraph, Pharmacophore& pharm)
{
	init(molgraph);
...
void Chem::HydrophobicFeatureGenerator::init(const MolecularGraph& molgraph)
{
	molGraph = &molgraph;
	...
	buildAtomTypeMask(molgraph, hAtomMask, AtomType::H);
}

void Chem::HydrophobicFeatureGenerator::genRingFeatures(Pharmacophore& pharm)
{
...
void Chem::HydrophobicFeatureGenerator::genGroupFeatures(Pharmacophore& pharm)
{
	std::size_t idx = 0;
...
void Chem::HydrophobicFeatureGenerator::genChainFeatures(Pharmacophore& pharm)
{
	std::size_t num_atoms = molGraph->getNumAtoms();
	...
	for (FragmentList::ConstElementIterator it = sssr.getElementsBegin(), end = sssr.getElementsEnd(); it != end; ++it) {
		...
		std::transform(ring.getAtomsBegin(), ring.getAtomsEnd(), 
					   std::back_inserter(chainAtoms), Util::AddressOf<const Atom>());
...
void Chem::HydrophobicFeatureGenerator::extractChain(const Atom& atom)
{
...
void Chem::HydrophobicFeatureGenerator::processChain(Pharmacophore& pharm)
{
	while (!chainAtoms.empty()) {
		...
		while (!chainAtoms.empty()) {
			const Atom* atom = chainAtoms.back();
...
Chem::Feature& Chem::HydrophobicFeatureGenerator::emitFeature(const AtomList& alist, Pharmacophore& pharm, 
...
bool Chem::HydrophobicFeatureGenerator::hasSubstWithMoreThan2Atoms(const Atom& atom, const Fragment& ring, bool first_level) const
{
	Atom::ConstAtomIterator a_it = atom.getAtomsBegin();
...
bool Chem::HydrophobicFeatureGenerator::calcHydWeightedCentroid(const AtomList& alist, Math::Vector3D& centroid) const
{
...
void Chem::HydrophobicFeatureGenerator::calcAtomHydrophobicities()
{
	std::size_t num_atoms = molGraph->getNumAtoms();
...
void Chem::HydrophobicFeatureGenerator::buildAtomMask(const AtomList& alist, Util::BitSet& mask) const
{
	mask.reset();
...
double Chem::HydrophobicFeatureGenerator::calcSummedHydrophobicity(const AtomList& alist) const
{
	double total_hyd = 0.0;
...
Chem::Fragment::SharedPointer Chem::HydrophobicFeatureGenerator::makeFragment(const AtomList& alist) const
{
...
bool Chem::HydrophobicFeatureGenerator::isChainEndAtom(const Atom& atom) const
{
