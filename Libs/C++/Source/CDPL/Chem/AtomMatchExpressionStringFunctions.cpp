			...
			switch (constraint.getID()) {
				...
				case AtomMatchConstraint::H_COUNT:
					createIntegralPropertyExpressionString(atom, constraint, expr_str, TOTAL_H_COUNT_SYMBOL, 
														   boost::bind(&getAtomCount, _1, boost::ref(molgraph), Chem::AtomType::H));
					break;
				...
				case AtomMatchConstraint::EXPLICIT_H_COUNT:
					createIntegralPropertyExpressionString(atom, constraint, expr_str, EXPLICIT_H_COUNT_SYMBOL, 
														   boost::bind(&getExplicitAtomCount, _1, boost::ref(molgraph), Chem::AtomType::H));
					break;
...
const std::string& Chem::getMatchExpressionString(const Atom& atom)
{
    return atom.getProperty<std::string>(AtomProperty::MATCH_EXPRESSION_STRING);
}

void Chem::setMatchExpressionString(Atom& atom, const std::string& expr_str)
{
    atom.setProperty(AtomProperty::MATCH_EXPRESSION_STRING, expr_str);
}

void Chem::clearMatchExpressionString(Atom& atom)
{
    atom.removeProperty(AtomProperty::MATCH_EXPRESSION_STRING);
}

bool Chem::hasMatchExpressionString(const Atom& atom)
{
    return atom.isPropertySet(AtomProperty::MATCH_EXPRESSION_STRING);
}
