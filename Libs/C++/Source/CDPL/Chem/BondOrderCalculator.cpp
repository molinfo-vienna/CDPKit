...
class Chem::BondOrderCalculator::AtomMatchExpression : public Chem::MatchExpression<Atom, MolecularGraph>
{
...
class Chem::BondOrderCalculator::BondMatchExpression : public Chem::MatchExpression<Bond, MolecularGraph>
{
...
private:
	const BondOrderCalculator& generator;
	const Util::STArray&      orders;
};


Chem::BondOrderCalculator::BondOrderCalculator(): undefOnly(true) {}

Chem::BondOrderCalculator::BondOrderCalculator(const MolecularGraph& molgraph, Util::STArray& orders, bool undef_only): undefOnly(undef_only) 
{
	calculate(molgraph, orders);
}

void Chem::BondOrderCalculator::undefinedOnly(bool undef_only) 
{
	undefOnly = undef_only;
}

bool Chem::BondOrderCalculator::undefinedOnly() const
{
	return undefOnly;
}

void Chem::BondOrderCalculator::calculate(const MolecularGraph& molgraph, Util::STArray& orders)
{
...
void Chem::BondOrderCalculator::calcFreeAtomValences(Util::STArray& orders)
{
...
double Chem::BondOrderCalculator::calcMappingScore(const AtomBondMapping& mapping) const
{
	double score = 0.0;
 	const BondMapping& bond_mapping = mapping.getBondMapping();
...
void Chem::BondOrderCalculator::assignConjPiSystemBondOrders(Util::STArray& orders)
{
...
void Chem::BondOrderCalculator::markPlanarPiBonds(Util::STArray& orders)
{
	for (BondList::const_iterator it = undefBonds.begin(), end = undefBonds.end(); it != end; ++it) {
		const Bond* bond = *it;
		std::size_t bond_idx = molGraph->getBondIndex(*bond);
...
void Chem::BondOrderCalculator::assignRemainingBondOrders(Util::STArray& orders)
{
...
void Chem::BondOrderCalculator::assignFragBondOrders(std::size_t depth, Util::STArray& orders)
{
...
double Chem::BondOrderCalculator::calcHybridizationMatchScore() 
{
	std::size_t frag_size = fragBondList.size();
...
void Chem::BondOrderCalculator::assignNbrBondOrders(const Atom& atom, Util::STArray& orders)
{
	Atom::ConstBondIterator bonds_end = atom.getBondsEnd();
	Atom::ConstAtomIterator a_it = atom.getAtomsBegin();
...
void Chem::BondOrderCalculator::postprocessGeometry(const Atom& atom, Util::STArray& orders)
{
	std::size_t atom_idx = molGraph->getAtomIndex(atom);
...
void Chem::BondOrderCalculator::fixRingAtomGeometries(const Fragment& ring)
{
...
Chem::BondOrderCalculator::Geometry Chem::BondOrderCalculator::perceiveInitialGeometry(const Atom& atom)
{
...
void Chem::BondOrderCalculator::getNeighborAtoms(const Atom& atom, AtomList& atom_list, const Atom* exclude_atom) const
{
	atom_list.clear();
...
std::size_t Chem::BondOrderCalculator::countBondsWithOrder(const Atom& atom, std::size_t order, const UIntTable& order_table) const 
{
	Atom::ConstBondIterator bonds_end = atom.getBondsEnd();
	Atom::ConstAtomIterator a_it = atom.getAtomsBegin();
...
void Chem::BondOrderCalculator::getUndefBondFragment(const Atom& atom, bool add_atoms, const Pred& bond_pred)
{
	if (add_atoms)
...
double Chem::BondOrderCalculator::calcAvgBondAngle(const Atom& atom, const AtomList& nbr_atoms) const
{
...
double Chem::BondOrderCalculator::calcDihedralAngle(const Atom& atom1, const Atom& atom2, const Atom& atom3, const Atom& atom4) const
{
	const Math::Vector3D& atom1_pos = get3DCoordinates(atom1);
	const Math::Vector3D& atom2_pos = get3DCoordinates(atom2);
...
double Chem::BondOrderCalculator::calcDihedralAngle(const Math::Vector3D& atom1_pos, const Math::Vector3D& atom2_pos, 
...
double Chem::BondOrderCalculator::calcAvgTorsionAngle(const Fragment& ring) const
{
	double angle_sum = 0.0;
	std::size_t rsize = ring.getNumAtoms();
...
bool Chem::BondOrderCalculator::isPlanarPiBond(const Bond& bond) const
{
