...
BOOST_AUTO_TEST_CASE(JMEMolecularGraphOutputHandlerTest)
{
    ...
    const DataOutputHandler<MolecularGraph>::SharedPointer handler = DataIOManager<MolecularGraph>::getOutputHandlerByFormat(Chem::DataFormat::JME);

    BOOST_CHECK(handler);

    BOOST_CHECK(handler->getDataFormat() == Chem::DataFormat::JME);

    BOOST_CHECK(DataIOManager<MolecularGraph>::getOutputHandlerByName("jme") == handler);
    BOOST_CHECK(DataIOManager<MolecularGraph>::getOutputHandlerByFileExtension("jme") == handler);
    ...
    BOOST_CHECK(iss);
    BOOST_CHECK(JMEMoleculeReader(iss).read(mol2));
