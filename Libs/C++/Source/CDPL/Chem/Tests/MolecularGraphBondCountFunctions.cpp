...
#include "CDPL/Internal/BondContainerFunctions.hpp"
...
{
	std::size_t count = Internal::getExplicitBondCount(molgraph, order);
...
{
	return (Internal::getExplicitHydrogenBondCount(molgraph) + getImplicitHydrogenCount(molgraph));
}

std::size_t Chem::getChainBondCount(const MolecularGraph& molgraph)
{
	return (Internal::getExplicitChainBondCount(molgraph) + getImplicitHydrogenCount(molgraph));
}
