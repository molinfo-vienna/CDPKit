...
unsigned int Chem::getCIPConfiguration(const Atom& atom)
{
    return atom.getProperty<unsigned int>(AtomProperty::CIP_CONFIGURATION);
}

void Chem::setCIPConfiguration(Atom& atom, unsigned int config)
{
    atom.setProperty(AtomProperty::CIP_CONFIGURATION, config);
}

void Chem::clearCIPConfiguration(Atom& atom)
{
    atom.removeProperty(AtomProperty::CIP_CONFIGURATION);
}

bool Chem::hasCIPConfiguration(const Atom& atom)
{
    return atom.isPropertySet(AtomProperty::CIP_CONFIGURATION);
}
...
unsigned int Chem::calcCIPConfiguration(const Atom& atom, const MolecularGraph& molgraph)
{
    ...
    std::sort(nbr_atoms, nbr_atoms + num_bonds,
			  boost::bind(std::greater<std::size_t>(),
						  boost::bind(&getCIPPriority, boost::bind(Util::Dereferencer<const Atom*, const Atom&>(), _1)), 
						  boost::bind(&getCIPPriority, boost::bind(Util::Dereferencer<const Atom*, const Atom&>(), _2))));
