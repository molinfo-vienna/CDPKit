		...
		for (MatchConstraintList::ConstElementIterator it = constr_list.getElementsBegin(); it != constr_end; ++it) {
			...
			switch (constraint.getID()) {
				...
				case AtomMatchConstraint::H_COUNT:
					expr_ptr = createIntegralPropertyMatchExpression<std::size_t>(atom, molgraph, constraint, boost::bind(&getAtomCount, _1, _2, AtomType::H));
					break;
				...
				case AtomMatchConstraint::EXPLICIT_H_COUNT:
					expr_ptr = createIntegralPropertyMatchExpression<std::size_t>(atom, molgraph, constraint, boost::bind(&getExplicitAtomCount, _1, _2, AtomType::H));
					break;
...
const Chem::MatchExpression<Chem::Atom, Chem::MolecularGraph>::SharedPointer& Chem::getMatchExpression(const Atom& atom)
{
    return atom.getProperty<MatchExpression<Atom, MolecularGraph>::SharedPointer>(AtomProperty::MATCH_EXPRESSION);
}

void Chem::setMatchExpression(Atom& atom, const MatchExpression<Atom, MolecularGraph>::SharedPointer& expr)
{
    atom.setProperty(AtomProperty::MATCH_EXPRESSION, expr);
}

void Chem::clearMatchExpression(Atom& atom)
{
    atom.removeProperty(AtomProperty::MATCH_EXPRESSION);
}

bool Chem::hasMatchExpression(const Atom& atom)
{
    return atom.isPropertySet(AtomProperty::MATCH_EXPRESSION);
}
