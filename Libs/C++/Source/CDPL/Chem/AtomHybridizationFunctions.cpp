...
/* 
 * AtomHybridizationFunctions.cpp 
 *
...
unsigned int Chem::getHybridization(const Atom& atom)
{
    return atom.getProperty<unsigned int>(AtomProperty::HYBRIDIZATION);
}

void Chem::setHybridization(Atom& atom, unsigned int hyb)
{
    atom.setProperty(AtomProperty::HYBRIDIZATION, hyb);
}

void Chem::clearHybridization(Atom& atom)
{
    atom.removeProperty(AtomProperty::HYBRIDIZATION);
}

bool Chem::hasHybridization(const Atom& atom)
{
    return atom.isPropertySet(AtomProperty::HYBRIDIZATION);
}

unsigned int Chem::perceiveHybridization(const Atom& atom)
{
    std::size_t bnd_order_counts[3] = { 0, 0, 0 };

    Atom::ConstBondIterator bonds_end = atom.getBondsEnd();
 
    for (Atom::ConstBondIterator b_it = atom.getBondsBegin(); b_it != bonds_end; ++b_it) {
    ...
    long free_val_el_count = calcValenceElectronCount(atom) - calcExplicitValence(atom) - impl_h_count;
