...
void Chem::calcFormalCharges(MolecularGraph& molgraph, bool overwrite)
{
    ...
    for (MolecularGraph::AtomIterator a_it = molgraph.getAtomsBegin(); a_it != atoms_end; ++a_it) {
        ...
        if (!overwrite && hasFormalCharge(atom))
            continue;

        setFormalCharge(atom, calcFormalCharge(atom, molgraph));
    }
}
