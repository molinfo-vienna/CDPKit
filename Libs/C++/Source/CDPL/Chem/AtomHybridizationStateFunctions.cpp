...
unsigned int Chem::getHybridizationState(const Atom& atom)
{
    return atom.getProperty<unsigned int>(AtomProperty::HYBRIDIZATION);
}

void Chem::setHybridizationState(Atom& atom, unsigned int hyb)
{
    atom.setProperty(AtomProperty::HYBRIDIZATION, hyb);
}

void Chem::clearHybridizationState(Atom& atom)
{
    atom.removeProperty(AtomProperty::HYBRIDIZATION);
}

bool Chem::hasHybridizationState(const Atom& atom)
{
