...
/* 
 * InteractionPharmacophoreFunctions.cpp 
 *
...
void Pharm::removeFeaturesWithoutInteractions(Pharmacophore& pharm, const FeatureMapping& iactions)
{
	for (std::size_t i = 0; i < pharm.getNumFeatures(); ) {
		if (!iactions.getValue(&pharm.getFeature(i)))
...
void Pharm::copyFeaturesWithInteractions(Pharmacophore& pharm, const FeatureMapping& iactions)
{
	for (FeatureMapping::ConstEntryIterator it = iactions.getEntriesBegin(), end = iactions.getEntriesEnd(); it != end; ) {
		const Feature& ftr1 = *it->first;
