	...
	namespace Math
	{
		...
		public:
			...
			bool containsLocalPoint(const V& pos) const {
				if (CoordinatesValueType(pos[0]) < (-getXExtent() / 2))
					return false;
		
				if (CoordinatesValueType(pos[0]) >= (getXExtent() / 2))
					return false;

				if (CoordinatesValueType(pos[1]) < (-getYExtent() / 2))
					return false;
		
				if (CoordinatesValueType(pos[1]) >= (getYExtent() / 2))
					return false;
	
				if (CoordinatesValueType(pos[2]) < (-getZExtent() / 2))
					return false;
		...
		T interpolateTrilinear(const RegularSpatialGrid<T, C, GD, XF>& grid, const V& pos, bool local_pos)
		{
			...
			typename GD::SizeType inds_p1[3];

			if (inds[0] + 1 >= grid.getSize1())
				inds_p1[0] = inds[0];
			else
				inds_p1[0] = inds[0] + 1;

			if (inds[1] + 1 >= grid.getSize2())
				inds_p1[1] = inds[1];
			else
				inds_p1[1] = inds[1] + 1;

			if (inds[2] + 1 >= grid.getSize3())
				inds_p1[2] = inds[2];
			else
				inds_p1[2] = inds[2] + 1;
			...
			C xd = (pos[0] - xyz0[0]) / grid.getXStepSize();
			C yd = (pos[1] - xyz0[1]) / grid.getYStepSize();
			C zd = (pos[2] - xyz0[2]) / grid.getZStepSize();

			T c00 = grid(inds[0], inds[1], inds[2]) * (1 - xd) + grid(inds_p1[0], inds[1], inds[2]) * xd;
			T c01 = grid(inds[0], inds[1], inds_p1[2]) * (1 - xd) + grid(inds_p1[0], inds[1], inds_p1[2]) * xd;
			T c10 = grid(inds[0], inds_p1[1], inds[2]) * (1 - xd) + grid(inds_p1[0], inds_p1[1], inds[2]) * xd;
			T c11 = grid(inds[0], inds_p1[1], inds_p1[2]) * (1 - xd) + grid(inds_p1[0], inds_p1[1], inds_p1[2]) * xd;
