...
# generates the MACCS key of the given molecular graph
def genMACCSKey(molgraph: Chem.MolecularGraph) -> Util.BitSet:
    Chem.calcBasicProperties(molgraph, False)  # calculate basic molecular properties (if not yet done)
   
    mk_gen = Descr.MACCSFingerprintGenerator() # create MACCS key generator instance
    fp = Util.BitSet()                         # create Util.BitSet instance storing the generated key

    # generate the MACCS key
    mk_gen.generate(molgraph, fp)
...
def parseArgs() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Generates 166 bit MACCS keys for given input molecules.')
