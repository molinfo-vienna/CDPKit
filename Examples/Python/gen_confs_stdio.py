...
def parseArgs() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Generates conformer ensembles for molecules read from standard input and writes the conformers to standard ouput.')

    parser.add_argument('-i',
                        dest='in_file_fmt',
                        required=True,
                        metavar='<string>',
                        help='Molecule input file format specification (= file extension)')
    parser.add_argument('-o',
                        dest='out_file_fmt',
                        required=True,
                        metavar='<string>',
                        help='Conformer ensemble output fileformat specification (= file extension)')
...
def main() -> None:
    ...
    handler = Chem.MoleculeIOManager.getInputHandlerByFileExtension(args.in_file_fmt)

    if handler is None:
        sys.exit(f'Error: molecule input data format \'{args.in_file_fmt}\' not supported')
    
    reader = handler.createReader(Base.IStream.STD_IN)

    # create writer for the generated conformer ensembles (format specified by file extension)
    handler = Chem.MolecularGraphIOManager.getOutputHandlerByFileExtension(args.out_file_fmt)

    if handler is None:
        sys.exit(f'Error: molecule output data format \'{args.out_file_fmt}\' not supported')
    
    writer = handler.createWriter(Base.OStream.STD_OUT)
    ...
    try:
        while reader.read(mol):
            ...
            if not args.quiet:
                print('- Generating conformers for molecule %s...' % mol_id, file=sys.stderr)
            ...
            try:
                ...
                if status != ConfGen.ReturnCode.SUCCESS and status != ConfGen.ReturnCode.TOO_MUCH_SYMMETRY:
                    if args.quiet:
                        print('Error: conformer ensemble generation for molecule %s failed: %s' % (mol_id, status_to_str[status]), file=sys.stderr)
                    else:
                        print(' -> Conformer ensemble generation failed: %s' % status_to_str[status], file=sys.stderr)

                elif not args.quiet:  # arrives here only if no severe error occurred
                    if status == ConfGen.ReturnCode.TOO_MUCH_SYMMETRY:
                        print(' -> Generated %s conformers (warning: too much top. symmetry - output ensemble may contain duplicates)' % str(num_confs), file=sys.stderr)
                    else:
                        print(' -> Generated %s conformer(s)' % str(num_confs), file=sys.stderr)
                ...
                sys.exit('Error: conformer ensemble generation or output for molecule %s failed:\n%s' % (mol_id, str(e)))
