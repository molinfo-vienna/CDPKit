...
# extracts all functional groups of a given input molecule and returns the MolProp.FunctionalGroupsList instance
def extractFunctionalGroups(in_mol: Chem.Molecule) -> MolProp.FunctionalGroupList:
    ...
    return func_grp_list

# returns the name of the functional group the given feature substructure is part of. If a functional group
# cannot be found (happens for, e.g., H and AR features) then the SMILES string of the feature substructure
# will be used as fallback
def getFunctionalGroupName(ftr_substruct: Chem.Fragment, func_grps: MolProp.FunctionalGroupList) -> str:
    for fg in func_grps:
        covers_atoms = True

        for atom in ftr_substruct.atoms:
            if atom not in fg:
                covers_atoms = False
                break

        if covers_atoms:
            return Chem.getName(fg)

    Chem.perceiveComponents(ftr_substruct, False)
        
    fg_name = Chem.generateSMILES(ftr_substruct)

    Chem.setName(ftr_substruct, fg_name)
    func_grps.addElement(ftr_substruct)
...
def outputInteractionData(ligand_idx: int, lig_mol: Chem.MolecularGraph, ia_ph4: Pharm.FeatureContainer, func_grps: MolProp.FunctionalGroupList, out_file) -> None:
    for ftr in ia_ph4:
        ...
        fg_name = getFunctionalGroupName(lig_substruct, func_grps)
                
        out_file.write(f'{ligand_idx}\t{ia_ph4.getFeatureIndex(ftr)}\t{Pharm.getType(ftr)}\t{lig_atom_inds}\t{fg_name}\t{Pharm.getEnvironmentResidueInfo(ftr)}\n')
        ...
        while lig_reader.read(lig_mol):
            ...
            try:
                ...
                if args.ia_out_file:                             # if desired, output interaction data
                    outputInteractionData(i - 1, lig_mol, ia_ph4, extractFunctionalGroups(lig_mol), ia_out_file)
